...
class MainWindow(tk.Tk):
    ...
    def radiobuttons_and_about_student_frame(self):
        ...
        info_and_mode_frame = tk.Frame(self,
    ...
    def create_radiobuttons(self, radio_frame):
        ...
        tk.Radiobutton(radio_frame, text="manually", variable=self.flag, value=0,
            command=lambda:self.select_mode("manually")).grid(column=0, row=1, sticky='w')

        tk.Radiobutton(radio_frame, text="auto", variable=self.flag, value=1,
            command=lambda:self.select_mode("auto")).grid(column=0, row=2, sticky='w')

    def select_mode(self, mode):
        if mode not in ["manually", "auto"]: return -1
        for i in ['a', 'b', 'c']:
    ...
    def random_set(self, a):
        s = set()
        while len(s) != a: s.add(str(random.randint(1, 10)))
        return s
    ...
    def generate_by_condition(self, flag):
        for i in ['a', 'b', 'c']:
            if flag:
                ...
                self.hash[i.title()] = '{}' if temp_set == '' else self.str_to_set(temp_set)
            temp_set = self.nametowidget('.mf.lf.f'+i+'.'+i).get()
            self.hash[i.title()] = self.random_set('{}' if temp_set == '' else int(temp_set))
    ...
    def generate_task(self):
        if self.flag.get() == 1: self.generate_by_condition(True)
        else: self.generate_by_condition(False)
    ...
    def about_student_button(self, frame):
        b = tk.Button(frame, width=18, height=1, fg='#d9073d', bg="#C0C0C0",
                      ...
                      font=("Segoe UI", 11, 'bold'), command=self.open_student_info_window)
        b.grid(padx=(320, 0), column=1, row=0, sticky='e', ipady=5)

    def left_and_right_frames_content(self, flag):
        c = (flag=='r')
        frame_ = tk.Frame(self.main_frame, height=100, width=320, name='rf' if c else 'lf')
        frame_.grid(column=0 if c else 2, row=0, sticky='e' if c else 'w', padx=(10,0) if c else (0, 0))
        ...
        for i in ['a', 'b', 'c']:
            ...
            tk.Entry(frame, state=tk.DISABLED, name=text_[3]+i).pack(side=tk.RIGHT)
    ...
    def generate_button(self):
        tk.Button(self.main_frame, text='Згенерувати множини', font=self.italic_font,
            width=60, height=1, bg="#C0C0C0", activebackground='grey',
            command=self.generate_task).grid(pady=(5, 0), column=0, row=1, columnspan=3)

    def task_condition(self):
        task_sets_frame = tk.Frame(self, highlightbackground="black", highlightthickness=1)
    ...
    def button_frame(self):
        button_frame = tk.Frame(self, highlightbackground="black", highlightthickness=1)
